...
- [ ] Keep `tts_text` as the only owner of playback, search, bookmarks, resume, and sentence indexing.
- [ ] Treat OCR geometry as a probabilistic alignment surface, never as guaranteed truth.
- [ ] Support degraded but honest behavior for PDFs where OCR text exists but sentence-level geometry is weak.
...
- [ ] The reader preserves deterministic reopen, bookmark, search, and playback behavior across OCR cache reuse and rebuilds.

## PDF Classes and Guarantees
- [ ] Define explicit OCR geometry quality classes:
- [ ] `ocr_high_trust`: OCR text and page geometry are stable enough for sentence-level mapping.
- [ ] `ocr_mixed_trust`: OCR text is usable, but geometry requires line/block fallback.
- [ ] `ocr_text_only`: OCR text is usable for Text-only/TTS, but geometry is too weak for reliable highlight sync.
- [ ] `ocr_failed_or_unusable`: OCR did not produce trustworthy text for TTS ownership.
...
- [ ] source markers distinguishing embedded text, OCR text, and mixed merged text
- [ ] Persist the contract in cache as a versioned OCR geometry artifact.
- [ ] Add tracing for OCR engine, OCR mode, per-page confidence summary, and total recognized token counts.
...
- [ ] Define the canonical OCR alignment artifact:
- [ ] `sentence_idx -> page_idx + rects[] + line_rects[] + block_rects[]`
- [ ] support one sentence spanning multiple lines
- [ ] support one sentence spanning multiple OCR blocks
- [ ] support one sentence crossing column boundaries only when confidence justifies it
- [ ] store confidence and fallback reason per mapping
- [ ] store contributing OCR token ids for auditability
- [ ] Build the artifact deterministically from canonical `tts_text` and OCR geometry, not from renderer DOM guesses alone.
- [ ] Persist the artifact alongside OCR text output and cache signatures.
...
- [ ] Keep search, bookmarks, and resume owned by canonical `tts_text`.
- [ ] Persist OCR-specific location metadata in bookmarks:
- [ ] page index
- [ ] rect set
- [ ] confidence tier
- [ ] fallback reason
- [ ] OCR token lineage if available
- [ ] Ensure reopening a scanned PDF restores the nearest stable cursor even if OCR alignment was rebuilt.
...
- [ ] Step 1: define and persist OCR geometry/cache schema
- [ ] Step 2: derive canonical `tts_text` with OCR token lineage
- [ ] Step 3: build page reading-order resolver
- [ ] Step 4: build sentence-to-geometry OCR alignment artifact
- [ ] Step 5: move OCR-first highlight rendering to rect overlays
- [ ] Step 6: wire search/bookmark/resume through OCR location metadata
